...
class Settings(BaseSettings):
    ...
    ollama_prompt_num_gpu: int = Field(-1, alias="OLLAMA_PROMPT_NUM_GPU", ge=-1)
    ollama_prompt_keep_alive: int = Field(0, alias="OLLAMA_PROMPT_KEEP_ALIVE", ge=0)
    ...
    default_steps: int = Field(28, alias="DEFAULT_STEPS", ge=8, le=60)
    generation_poll_seconds: float = Field(0.5, alias="GENERATION_POLL_SECONDS", ge=0.1, le=5)
    ...
    prompt_translation_total_timeout_seconds: float = Field(45, alias="PROMPT_TRANSLATION_TOTAL_TIMEOUT_SECONDS", gt=0)
    ...
    inpaint_blend_denoise: float = Field(0.32, alias="INPAINT_BLEND_DENOISE", ge=0, le=0.70)
    inpaint_blend_steps: int = Field(16, alias="INPAINT_BLEND_STEPS", ge=0, le=40)
    inpaint_blend_mask_expand: int = Field(24, alias="INPAINT_BLEND_MASK_EXPAND", ge=0, le=128)
    inpaint_mask_blur: int = Field(0, alias="INPAINT_MASK_BLUR", ge=0, le=31)
    inpaint_color_match: float = Field(0.55, alias="INPAINT_COLOR_MATCH", ge=0, le=1)
    fooocus_inpaint_head: str = Field("", alias="FOOOCUS_INPAINT_HEAD")
    fooocus_inpaint_patch: str = Field("", alias="FOOOCUS_INPAINT_PATCH")
    anima_clip_name: str = Field("qwen_3_06b_base.safetensors", alias="ANIMA_CLIP_NAME")
